...
// Returns a Buffer — used for email attachments and Route Handler responses
function generateToBuffer(drawFn: (doc: Doc) => void): Promise<Buffer> {
...
function drawHeader(doc: Doc, docTitle: string, docRef: string): number {
  // Navy gradient-style header band
  doc.rect(0, 0, PW, 72).fill(C.navy);
  doc.rect(0, 72, PW, 6).fill(C.amber);

  // Brand name
  doc.fillColor(C.white).font('Helvetica-Bold').fontSize(20).text('ACCESSIBLEXPRESS', ML, 18);
  doc.fillColor(C.amber).font('Helvetica').fontSize(8).text('GLOBAL LOGISTICS & FREIGHT', ML, 42);

  // Document title (right-aligned)
  doc.fillColor(C.white).font('Helvetica-Bold').fontSize(15).text(docTitle, 0, 18, { align: 'right', width: PW - MR });
  doc.fillColor(C.amber).font('Helvetica').fontSize(8).text(docRef, 0, 42, { align: 'right', width: PW - MR });

  return 90; // y after header
}

function drawSectionBar(doc: Doc, y: number, title: string): number {
  doc.rect(ML, y, CW, 18).fill(C.navy);
  doc.fillColor(C.white).font('Helvetica-Bold').fontSize(7.5).text(title.toUpperCase(), ML + 8, y + 5.5, {
    width: CW - 16,
  });
  return y + 18;
}

function drawField(doc: Doc, x: number, y: number, w: number, label: string, value?: string | number): void {
  doc.fillColor(C.muted).font('Helvetica').fontSize(6.5).text(label.toUpperCase(), x, y, { width: w });
...
function drawFooters(doc: Doc, trackingNumber: string): void {
  ...
  for (let i = 0; i < count; i++) {
    ...
    doc
      ...
      .font('Helvetica')
      .fontSize(7)
      .text(`${trackingNumber}  •  Generated ${new Date().toUTCString()}  •  accessiblexpress.com`, ML, fy + 10);
    doc
      .fillColor(C.muted)
      .font('Helvetica')
      .fontSize(7)
...
function drawParties(
  ...
  y: number,
  leftLabel: string,
  left: TemplateShipment['sender'],
  rightLabel: string,
  right: TemplateShipment['recipient']
): number {
  const mid = ML + CW / 2;
  const colW = CW / 2 - 12;
  ...
  const addr = (p: TemplateShipment['sender']) =>
  ...
  // Vertical divider
  doc.moveTo(mid, y - 4).lineTo(mid, y + 108).strokeColor(C.border).lineWidth(0.5).stroke();
...
function drawAWB(doc: Doc, shipment: TemplateShipment): void {
  ...
  doc.fillColor(C.muted).font('Helvetica').fontSize(7).text('AIR WAYBILL NUMBER', ML + 14, y + 8);
  doc.fillColor(C.navy).font('Helvetica-Bold').fontSize(22).text(shipment.trackingNumber || '', ML + 14, y + 18);
  ...
  y += 8;
  doc.fillColor(C.text).font('Helvetica').fontSize(9).text(shipment.contents || 'General Cargo', ML + 8, y, {
    width: CW - 16,
  });
  ...
  drawField(doc, ML + 8, y, tw, 'Issue Date', shipment.createdAt ? new Date(shipment.createdAt).toDateString() : '—');
  ...
  doc.fillColor(C.muted).font('Helvetica').fontSize(7).text('Signature & Date', ML + 10, y + 40);
  y += 60;
...
function drawInvoice(doc: Doc, shipment: TemplateShipment): void {
  ...
  drawField(doc, ML + 14 + mw + 10, y + 8, mw, 'Invoice Date', shipment.createdAt ? new Date(shipment.createdAt).toDateString() : '—');
  ...
  hdrs.forEach((h, i) => {
    ...
    doc.fillColor(C.white).font('Helvetica-Bold').fontSize(7.5).text(h, cols[i], y + 6, { width: w });
  });
  y += 20;
  ...
  doc.fillColor(C.amber).font('Helvetica-Bold').fontSize(9).text('TOTAL DECLARED VALUE', cols[0], y + 9);
  doc
    .fillColor(C.white)
    .font('Helvetica-Bold')
    .fontSize(13)
  ...
  doc.fillColor(C.muted).font('Helvetica').fontSize(7).text('Authorised Signature & Company Stamp', ML + 10, y + 38);
  y += 60;
...
function drawPackingList(doc: Doc, shipment: TemplateShipment): void {
  ...
  drawField(doc, ML + 14 + mw + 10, y + 8, mw, 'Date Prepared', shipment.createdAt ? new Date(shipment.createdAt).toDateString() : '—');
  drawField(doc, ML + 14 + (mw + 10) * 2, y + 8, mw, 'Status', (shipment.status || '').replace(/_/g, ' ').toUpperCase());
  y += 58;
  ...
  phdrs.forEach((h, i) => {
    ...
    doc.fillColor(C.white).font('Helvetica-Bold').fontSize(7.5).text(h, pcols[i], y + 6, { width: w });
  });
  y += 20;
  ...
  doc.fillColor(C.amber).font('Helvetica-Bold').fontSize(8.5).text('TOTALS', pcols[0], y + 8);
  doc
    .fillColor(C.white)
    .font('Helvetica-Bold')
    .fontSize(8.5)
  ...
  const noteLines = doc.heightOfString(noteText, { width: CW - 20 });
  const noteBoxH = Math.max(40, noteLines + 20);
  doc.rect(ML, y, CW, noteBoxH).fill(C.light).strokeColor(C.border).lineWidth(0.5).stroke();
  doc.fillColor(C.text).font('Helvetica').fontSize(8.5).text(noteText, ML + 10, y + 10, { width: CW - 20 });
  y += noteBoxH + 10;
...
export const generateAWB = async (shipment: TemplateShipment): Promise<GeneratedDocument> => ({
  buffer: await generateToBuffer((doc) => drawAWB(doc, shipment)),
  filename: `AWB-${shipment.trackingNumber}.pdf`,
});

export const generateInvoice = async (shipment: TemplateShipment): Promise<GeneratedDocument> => ({
  buffer: await generateToBuffer((doc) => drawInvoice(doc, shipment)),
  filename: `Invoice-${shipment.trackingNumber}.pdf`,
});

export const generatePackingList = async (shipment: TemplateShipment): Promise<GeneratedDocument> => ({
  buffer: await generateToBuffer((doc) => drawPackingList(doc, shipment)),
  filename: `PackingList-${shipment.trackingNumber}.pdf`,
});

export const generateAllToBuffers = async (
  shipment: TemplateShipment
): Promise<{ awb: Buffer; invoice: Buffer; packingList: Buffer }> => {
